...
export default function OAuthAuthorizePage(): React.ReactElement {
  ...
  async function validateRequest() {
    ...
    try {
      ...
      if (!result.valid) {
        setPageState('error')
        setErrorMsg(result.error_description || result.error || t('oauth.consent.invalidRequest'))
        return
      }
      ...
      setPageState('error')
      setErrorMsg(t('oauth.consent.validateFailed'))
    }
  }
  ...
  async function submitConsent(approved: boolean, scopes: string[] = selectedScopes) {
    ...
    try {
      ...
      setPageState('error')
      setErrorMsg(t('oauth.consent.authorizeFailed'))
      setSubmitting(false)
    }
  }
  ...
  const scopesByGroup = React.useMemo(() => {
    ...
    for (const s of requested) {
      ...
      const group = keys ? t(keys.groupKey) : t('oauth.consent.otherGroup')
        ...
        <div className="min-h-screen flex items-center justify-center" style={{ background: 'var(--bg-primary)' }}>
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>
              {pageState === 'auto_approving' ? t('oauth.consent.authorizing') : t('common.loading')}
            </p>
          </div>
        </div>
    )
    ...
    return (
        <div className="min-h-screen flex items-center justify-center p-4" style={{ background: 'var(--bg-primary)' }}>
          <div className="w-full max-w-sm rounded-xl shadow-lg p-8 space-y-4 text-center" style={{ background: 'var(--bg-card)' }}>
            ...
            <h1 className="text-xl font-semibold" style={{ color: 'var(--text-primary)' }}>{t('oauth.consent.errorTitle')}</h1>
        ...
        <div className="min-h-screen flex items-center justify-center p-4" style={{ background: 'var(--bg-primary)' }}>
          <div className="w-full max-w-sm rounded-xl shadow-lg p-8 space-y-5" style={{ background: 'var(--bg-card)' }}>
            <div className="text-center space-y-2">
              ...
              <h1 className="text-xl font-semibold" style={{ color: 'var(--text-primary)' }}>{t('oauth.consent.signInTitle')}</h1>
              <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>
                <strong>{validation?.client?.name || clientId}</strong> {t('oauth.consent.signInMessage')}
              </p>
            </div>
            <button
        ...
        <div className="w-full max-w-2xl rounded-xl shadow-lg overflow-hidden flex flex-col sm:flex-row" style={{ background: 'var(--bg-card)' }}>
          ...
          <div className="sm:w-64 sm:flex-shrink-0 flex flex-col px-8 py-8 sm:border-r" style={{ borderColor: 'var(--border-primary)' }}>
            <div className="flex-1 space-y-4">
              ...
              <div>
                <p className="text-xs font-medium uppercase tracking-wide mb-1" style={{ color: 'var(--text-tertiary)' }}>{t('oauth.consent.requestTitle')}</p>
                <h1 className="text-lg font-semibold leading-snug" style={{ color: 'var(--text-primary)' }}>
                  {validation?.client?.name || clientId}
                </h1>
                <p className="text-sm mt-2" style={{ color: 'var(--text-secondary)' }}>
                  {t('oauth.consent.requestMessage')}
                </p>
              </div>
            </div>
              ...
              <button
                ...
                {submitting
                    ...
                    : validation?.scopeSelectable && selectedScopes.length === 0
                        ? t('oauth.consent.selectScope')
                        : validation?.scopeSelectable
                            ? t(selectedScopes.length === 1 ? 'oauth.consent.approveScopesOne' : 'oauth.consent.approveScopes', { count: selectedScopes.length })
            ...
            <div className="space-y-6">
              {Object.keys(scopesByGroup).length > 0 && (
                  <div>
                    <p className="text-xs font-medium uppercase tracking-wide mb-4" style={{ color: 'var(--text-tertiary)' }}>
                      {validation?.scopeSelectable ? t('oauth.consent.chooseScopes') : t('oauth.consent.scopesRequested')}
                    </p>
              ...
              <div>
                <p className="text-xs font-medium uppercase tracking-wide mb-3" style={{ color: 'var(--text-tertiary)' }}>
                  {t('oauth.consent.alwaysIncluded')}
                </p>
                <div className="space-y-1.5">
                  {[
                    { name: 'list_trips',       desc: t('oauth.consent.listTripsDesc') },
                    { name: 'get_trip_summary', desc: t('oauth.consent.tripSummaryDesc') },
                  ].map(({ name, desc }) => (
