...
import ArtikelZeile from './ArtikelZeile'
import type { PackingItem } from '../../types'
import type { CategoryAssignee, PackingBag, TripMember } from './types'
import { katColor } from './types'
    ...
    <div style={{ marginBottom: 6, background: 'var(--bg-card)', borderRadius: 14, border: '1px solid var(--border-secondary)', overflow: 'visible' }}>
      ...
      {offen && (
        <div style={{ padding: '4px 4px 6px' }}>
          {items.map(item => {
            const itemBagId = (item as PackingItem & { bag_id?: number | null }).bag_id
            const bagMembers = itemBagId != null ? bags?.find(b => b.id === itemBagId)?.members?.length : undefined
            const itemAssigneeCount = bagMembers ?? assigneeCount
            return (
              <ArtikelZeile key={item.id} item={item} tripId={tripId} categories={allCategories} onCategoryChange={() => {}} bagTrackingEnabled={bagTrackingEnabled} bags={bags} onCreateBag={onCreateBag} canEdit={canEdit} assigneeCount={itemAssigneeCount} />
            )
          })}
          {canEdit && (showAddItem ? (
