...
import { CheckSquare, Square, Trash2, Plus, X, Pencil, Package } from 'lucide-react'
...
import { katColor, PACKING_PLACEHOLDER_NAME } from './types'
...
interface ArtikelZeileProps {
  ...
  canEdit?: boolean
  /** Number of members assigned to this item's category or bag. When > 1, show the checked-by badge. */
  assigneeCount?: number
}

export default function ArtikelZeile({ item, tripId, categories, bagTrackingEnabled, bags = [], onCreateBag, canEdit = true, assigneeCount = 0 }: ArtikelZeileProps) {
  ...
  const showCheckedBy = !!item.checked && item.checked_by_user_id != null && assigneeCount > 1

  // PackingItem has optional bag/weight fields populated when bag tracking is on
  const itemWithBag = item as PackingItem & { bag_id?: number | null; weight_grams?: number | null }
      ...
      {bagTrackingEnabled && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 2, border: '1px solid var(--border-primary)', borderRadius: 8, padding: '3px 6px', background: 'transparent' }}>
            <input
              type="text" inputMode="numeric"
              value={itemWithBag.weight_grams ?? ''}
              readOnly={!canEdit}
              onChange={async e => {
                if (!canEdit) return
                const raw = e.target.value.replace(/[^0-9]/g, '')
                const v = raw === '' ? null : parseInt(raw)
                try { await updatePackingItem(tripId, item.id, { weight_grams: v } as Record<string, unknown>) } catch { /* swallow */ }
              }}
              placeholder="—"
          ...
          <div style={{ position: 'relative' }}>
            <button
              ...
              style={{
                ...
                border: itemWithBag.bag_id ? `2.5px solid ${bags.find(b => b.id === itemWithBag.bag_id)?.color || 'var(--border-primary)'}` : '2px dashed var(--border-primary)',
                background: itemWithBag.bag_id ? `${bags.find(b => b.id === itemWithBag.bag_id)?.color || 'var(--border-primary)'}30` : 'transparent',
              }}
            >
              {!itemWithBag.bag_id && <Package size={9} style={{ color: 'var(--text-faint)' }} />}
            </button>
            {showBagPicker && (
              <div style={{
                ...
                {itemWithBag.bag_id && (
                  <button onClick={async () => { setShowBagPicker(false); try { await updatePackingItem(tripId, item.id, { bag_id: null } as Record<string, unknown>) } catch { /* swallow */ } }}
                ...
                {bags.map(b => (
                  <button key={b.id} onClick={async () => { setShowBagPicker(false); try { await updatePackingItem(tripId, item.id, { bag_id: b.id } as Record<string, unknown>) } catch { /* swallow */ } }}
                    style={{
                      display: 'flex', alignItems: 'center', gap: 7, width: '100%', padding: '6px 10px',
                      background: itemWithBag.bag_id === b.id ? 'var(--bg-tertiary)' : 'none',
                      border: 'none', cursor: 'pointer', fontSize: 12, fontFamily: 'inherit', color: 'var(--text-secondary)', borderRadius: 7,
                    }}
                    onMouseEnter={e => { if (itemWithBag.bag_id !== b.id) e.currentTarget.style.background = 'var(--bg-tertiary)' }}
                    onMouseLeave={e => { if (itemWithBag.bag_id !== b.id) e.currentTarget.style.background = 'none' }}>
                      ...
                      <input autoFocus value={bagInlineName} onChange={e => setBagInlineName(e.target.value)}
                        onKeyDown={async e => {
                          if (e.key === 'Enter' && bagInlineName.trim()) {
                            ...
                            if (newBag) { try { await updatePackingItem(tripId, item.id, { bag_id: newBag.id } as Record<string, unknown>) } catch { /* swallow */ } }
